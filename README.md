...
- `Option + C`：打开或切到 ChatGPT
- `Option + E`：打开或切到邮件
- `Option + M`：打开或切到音乐
- `Option + B`：打开或切到 Google Chrome
- `Option + Shift + Z`：唤起快捷虾面板
...
菜单栏可以暂停全部切换快捷键，暂停状态会在重启后保留；`Option + Shift + Z` 面板入口始终可用，方便随时恢复。
...
首次用快捷键切换 App 时，系统可能会提示授予“辅助功能”权限。允许后，快捷虾可以在跳转到目标桌面后把目标 App 的窗口抬到最前并聚焦；唤起面板本身不再依赖辅助功能权限。
